...
async function writeHeaders(accessToken: string, spreadsheetId: string): Promise<void> {
  ...
  await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Tickets!A1:J1?valueInputOption=USER_ENTERED`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
...
export async function fetchSpreadsheetTickets(accessToken: string, spreadsheetId: string): Promise<Ticket[]> {
  const response = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Tickets!A2:J1000`, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  });
...
export async function syncSpreadsheetTickets(accessToken: string, spreadsheetId: string, tickets: Ticket[]): Promise<void> {
  ...
  await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Tickets!A2:J1000:clear`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  ...
  // 3. Write rows
  const response = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/Tickets!A2:J${tickets.length + 1}?valueInputOption=USER_ENTERED`, {
    method: 'PUT',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({
      range: `Tickets!A2:J${tickets.length + 1}`,
      majorDimension: 'ROWS',
      values: rows,
    }),
  });
